...
- **Formula**:
    ...
    `$
- **Calculation**: Count the number of relevant items in the top `k` retrieved
  results; divide that by the first 'k' relevant items.
- **Example**: Suppose there are 4 relevant documents for a given query.
  Suppose our system retrieves 3 of them in the top 5 results (`k=5`).
  Recall@5 is `3 / 4 = 0.75`.
```python
recall_at_k(
